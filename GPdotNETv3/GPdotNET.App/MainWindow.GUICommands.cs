...
namespace GPdotNET.App
{
    public partial class MainWindow
    {
        ...
        private void InitStartPanel()
        {
            ...
            pbLogoHor.Image = Utility.LoadImageFromName("GPdotNET.App.Resources.gpLogo_350x134pix.png");
        ...
        private void runProgram()
        {
            switch (_GPModel)   
            {
                ...
                case GPModelType.TimeSeries:
                    if (PrepareGP())
                        _run(_runPanel, GPRunType.GPModelling);
                    break;
                ...
                case GPModelType.TSP:
                    if(PrepareTSP())
                        _run(_tspPanel, GPRunType.GATSPProblem);
                    break;
                case GPModelType.ALOC:
                    if (PrepareALOC())
                        _run(_alocPanel, GPRunType.GAALOCProblem);
                    break;
                case GPModelType.Transport:
                    if (PrepareTrans())
                        _run(_alocPanel, GPRunType.GATransport);
                    break;
                default:
                    break;
            }
        ...
        private void optimizeProgram()
        {
            if (_GPModel == Tool.Common.GPModelType.AnaliticFunctionOptimization || _GPModel == Tool.Common.GPModelType.SymbolicRegressionWithOptimization)
            {
               ...
               if(PrepareGA())
                _run(_optimizePanel, GPRunType.GAOptimization);
            }
        ...
        /// 
        /// </summary>
        private void _run(BaseRunPanel pn, GPRunType gpTYpe = GPRunType.GPModelling)
        {
            try
            {
                Task tsk;
                ...
                UpdateGUI(1);
                if (gpTYpe == GPRunType.GPModelling)
                    tsk = new Task(() => _mainFactory.StartEvolution(tv, tt));
                else if (gpTYpe == GPRunType.GAOptimization)
                    tsk = new Task(() => _secondFactory.StartEvolution(tv, tt));
                else if (gpTYpe == GPRunType.GATSPProblem)
                    tsk = new Task(() => _mainFactory.StartEvolution(tv, tt));
                else if (gpTYpe == GPRunType.GAALOCProblem)
                    tsk = new Task(() => _mainFactory.StartEvolution(tv, tt));
                else
                    tsk = new Task(() => _mainFactory.StartEvolution(tv, tt));
                //
        ...
        private void ReportEvolution(ProgressIndicatorEventArgs e)
        {
            ...
            {
                _tspPanel.ReportProgress(e.CurrentEvolution, e.AverageFitness, e.BestChromosome, (int)e.ReportType);
            }
            if (_alocPanel != null && _runningEngine == 1)
            {
                _alocPanel.ReportProgress(e.CurrentEvolution, e.AverageFitness, e.BestChromosome, (int)e.ReportType);
            }
