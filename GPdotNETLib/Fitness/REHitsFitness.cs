...
{
    /// <summary>
    /// GPdotNET 4.0 implements the Relative/Hits fitness function. For some Symbolic Regression problems 
    ...
    /// </summary>
    [Serializable]
    public class REHitsFitness:IFitnessFunction
    {
        ...
        public void Evaluate(List<ushort> lst, GPFunctionSet gpFunctionSet, GPTerminalSet gpTerminalSet, GPChromosome c)
        {
            ...
            for (int i = 0; i < gpTerminalSet.RowCount; i++)
            {
                ...
                if (Math.Abs((y - gpTerminalSet.TrainingData[i][indexOutput]) / gpTerminalSet.TrainingData[i][indexOutput]) > p)
                    rowFitness += 0;
                else
                    rowFitness += 1;
