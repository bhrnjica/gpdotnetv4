...
{
    /// <summary>
    ///GPdotNET 4.0 implements the Relative Squared Error (RSE) fitness. The RSE fitness function of GPdotNET is, as expected,
    ///based on the standard relative squared error, which, on its turn, is based on the absolute error.The relative squared 
    ///error is relative to what it would have been if a simple predictor had been used. More specifically, this simple 
    ///predictor is just the average of the actual values. Thus, the relative squared error takes the total squared error
