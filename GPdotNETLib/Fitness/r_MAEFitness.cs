...
namespace GPdotNETLib
{
    ...
    /// relative error rather than the absolute) fitness function. The rMAE fitness function of GPdotNET is, as expected, 
    /// based on the standard AverageValue absolute error, which is usually based on the absolute error, but obviously the 
    /// relative error can also be used in order to create a slightly different fitness measure.
    /// </summary>
    [Serializable]
