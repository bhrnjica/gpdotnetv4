...
namespace GPdotNETLib
{
    ...
    public class GPFunctionSet
    {
        ...
        public double Evaluate(List<ushort> tokens, GPTerminalSet gpTerminalSet, int numRow)
        {
        ...
        public string DecodeWithOptimisationExpression(List<ushort> tokens, GPTerminalSet gpTerminalSet)
        {
            return "";
        ...
        public string DecodeExpression(List<ushort> tokens, GPTerminalSet gpTerminalSet)
        {
            //Prepare chromoseme for evaluation
            //    List<int> tokens = new List<int>();
            //    FunctionTree.ToListExpression(tokens, c.Root);
        ...
        public string DecodeExpressionInExcellForm(List<ushort> tokens, GPTerminalSet gpTerminalSet)
        {
            //Prepare chromoseme for evaluation
            //    List<int> tokens = new List<int>();
            //    FunctionTree.ToListExpression(tokens, c.Root);
        ...
        public ushort GenerateGene()
        {
            // give more chance to be function node
            return GenerateGene((GPPopulation.rand.Next(4) != 3));
        }
        public ushort GenerateGene(bool isFunction)
        {
            ushort value = 0;
            //chech if number of terminals exceed 999 number
            if (terminals.Count > 999)
                throw new Exception("Maximum number of terminals must be less than 1000");

            // index greather than 1999 treated as functions
            if (isFunction)
            {
                ushort f = (ushort)GPPopulation.rand.Next(functions.Count);
                value = (ushort)(2000 + f);
            }
            //Index number from 1000 to 2000 treated as terminals
            else
            {
                //Slucajno biramo jedan od terminala
                ushort f = (ushort)GPPopulation.rand.Next(terminals.Count);
                value = (ushort)(1000 + f);
            }
            return value;
        }
